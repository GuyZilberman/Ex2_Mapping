...
def build_LLA_GPS_trajectory(dataset):
    ...
    for gps_imu in dataset.get_gps_imu():
        gps_imu = gps_imu[0]
        currect_lla = np.array([gps_imu.lon, gps_imu.lat,gps_imu.alt])
...
def build_GPS_trajectory(dataset):
    ...
    for idx, (gps_imu, timestamp) in enumerate(zip(dataset.get_gps_imu(), dataset.get_timestamps())):
        ...
        curr_yaw_vf_vl_wz = np.array([gps_imu.yaw, gps_imu.vf, gps_imu.vl, gps_imu.wz])
...
def normalize_angles_array(angles):
    ...
    for i in range(angles.shape[0]):
        z[i] = normalize_angle(angles[i]) #normalize_angle(angles[i]) #np.unwrap(np.array([angles[i]]))[0]
    return z
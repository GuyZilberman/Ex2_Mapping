...
def percentage_within_boundary(err_i, boundary_i):
    within_boundary=0
    for index, boundary in enumerate(boundary_i):
        if abs(err_i[index]) < boundary:
            within_boundary += 1

    total_count = len(err_i)
    percentage = (within_boundary / total_count) * 100
    return percentage
...
def plot_error(err_cov_x, err_cov_y, err_cov_yaw=None):
    ...
    percentagex=percentage_within_boundary(err_x, cov_x)
    std_value = 1  # Example standard deviation value
    ...
    ax1.set_title(
    r"Error throughout the path along the x axis: {:.2f}% inside $[{}\sigma, {}\sigma]$".format(percentagex, "-", "+"),
    fontsize=20
    )
    ax1.set_xlabel("Frame #", fontsize=20)
    ...
    percentagey=percentage_within_boundary(err_y, cov_y)
    ax2.plot(range(err_y.shape[0]), err_y, 'b')
    ax2.plot(range(len(cov_y)), cov_y, 'r')
    ax2.plot(range(len(cov_y)), [-a for a in cov_y], 'r')
    ax2.set_title(
    r"Error throughout the path along the y axis: {:.2f}% inside $[{}\sigma, {}\sigma]$".format(percentagey, "-", "+"),
    fontsize=20
    )

    ax2.set_xlabel("Frame #", fontsize=20)
    ax2.set_ylabel("Error (y_gt - y_predicted) [meters]", fontsize=20)
    ax2.legend(["Error (y_gt - y_predicted) [meters]", "Predicted variance"], prop={"size": 20}, loc="best")
    ax2.grid()  # Add grid to ax1

    if err_cov_yaw:
        err_yaw, cov_yaw = err_cov_yaw
        percentageYAW=percentage_within_boundary(err_y, cov_y)
        ...
        ax3.set_title(
        r"Error throughout the path in yaw: {:.2f}% inside $[{}\sigma, {}\sigma]$".format(percentageYAW, "-", "+"),
        fontsize=20
        )

        ax3.set_xlabel("Frame #", fontsize=20)
        ax3.set_ylabel("Error (yaw_gt - yaw_predicted) [rad]", fontsize=20)
        ax3.legend(["Error (yaw_gt - yaw_predicted) [meters]", "Predicted variance"], prop={"size": 20}, loc="best")
        ax3.grid()  # Add grid to ax1
...
def plot_trajectory_comparison_kf_ekf(enu, enu_predicted_ekf, enu_predicted_kf):
    """
    Plots the ground truth path, the predicted path and the predicted dead reckoning path
    Args:
        enu: numpy array where the rows are (x, y) it may contain the heading too without effect
    ...
    fig, ax = plt.subplots()
    ax.plot(enu[:, 0], enu[:, 1], 'b')
    ax.plot(enu_predicted_ekf[:, 0], enu_predicted_ekf[:, 1], 'r')
    ax.plot(enu_predicted_kf[:, 0], enu_predicted_kf[:, 1], 'g')
    ax.set_aspect('equal', adjustable='box')
    ax.set_title("Comparison of the ground truth trajectory, EKF predicted trajectory and KF predicted trajectory", fontsize=20)
    ax.set_xlabel("East [meters]", fontsize=20)
    ax.set_ylabel("North [meters]", fontsize=20)
    ax.legend(["Ground truth trajectory", "EKF predicted", "KF predicted"], prop={"size": 20}, loc="best")
    ax.grid()  # Add grid to ax
...
def plot_trajectory_with_noise(pos_xy_gt, pos_xy_noise, title, xlabel, ylabel, legend_gt, legend_noise):
   ...
   # ax.scatter(pos_xy_noise[:, 0], pos_xy_noise[:, 1],c='k',marker='.',s=4)
    ax.set_aspect('equal', adjustable='box')
    ax.set_title(title, fontsize=20)
    ax.set_xlabel(xlabel, fontsize=20)
    ax.set_ylabel(ylabel, fontsize=20)
...
def build_animation(X_Y0, X_Y1, X_Y2, x_xy_xy_y, title, xlabel, ylabel, label0, label1, label2):
    ...
    def init():
        ...
        x_min_1 = np.min(X_Y1[:, 0]) - margin
        x_max_1 = np.max(X_Y1[:, 0]) + margin
        y_min_1 = np.min(X_Y1[:, 1]) - margin
        y_max_1 = np.max(X_Y1[:, 1]) + margin

        x_min_2 = np.min(X_Y2[:, 0]) - margin
        x_max_2 = np.max(X_Y2[:, 0]) + margin
        y_min_2 = np.min(X_Y2[:, 1]) - margin
        y_max_2 = np.max(X_Y2[:, 1]) + margin

        x_min = min([x_min,x_min_1,x_min_2])
        x_max = max([x_max,x_max_1,x_max_2])
        y_min = min([y_min,y_min_1,y_min_2])
        y_max = max([y_max,y_max_1,y_max_2])
